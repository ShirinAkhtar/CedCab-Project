...
if(isset($_REQUEST["Aid"]))
{
    ...
    header('Location: requestApproved.php');
}
			...
			<table id="exam_data_table" class="table table-bordered table-striped table-hover">
				...
				<?php  foreach($store1 as $key=> $value){
							...
							<tr>
								...
								<td><?php echo 'Unblock' ?></td>
								<?php if ($value['Isblock'] == "1") { ?>
									<td><a href="requestApproved.php?action=access&Did=<?php echo $value['Uid'];?>" onClick="return confirm('Are you sure you want to block?')"  class="edit_btn1" name="access_granted">Block</a> </td>
								<?php } else 
											{ ?>
								<td><a href="requestApproved.php?action=access&Aid=<?php echo $value['Uid'];?>" class="edit_btn" name="access_granted">Block</a> </td>
